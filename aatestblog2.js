...
// dengan parameter tracking referrer + utm_campaign otomatis.
...
(function() {
  ...
  const utmCampaign = 'utm_campaign=autoredirect_' + Date.now(); // timestamp unik

  // Gabungkan parameter ke URL target
  const separator = randomTarget.includes('?') ? '&' : '?';
  const finalUrl = `${randomTarget}${separator}${refParam}&${utmSource}&${utmMedium}&${utmCampaign}`;

  // Redirect otomatis ke URL acak
